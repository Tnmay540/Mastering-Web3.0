...
// new test cases for bonus assignment code are added at the last

describe("TicketSale2", function() {
  let ticketSale;
  let owner;
  let addr1;
  let addr2;
  let addr3;
  beforeEach(async () => {
    const TicketSale = await ethers.getContractFactory("TicketSale2");
  ...
  it("should allow a person to submit an offer for their ticket if the offer price is within expected range", async function() {
    ...
    const offer = await ticketSale.offers(1);
  ...
  it("should multiple offers submitted by different persons at the same time", async function() {
    await ticketSale.connect(addr1).buyTicket({ value: 100 });
    await ticketSale.connect(addr2).buyTicket({ value: 100 });
    await ticketSale.connect(addr1).submitOffer(1, 120);
    expect(ticketSale.connect(addr2).submitOffer(2, 80));
    expect(ticketSale.offers(1).offerer == (addr1));
    expect(ticketSale.offers(2).offerer == (addr2));
  });
  ...
  it("should not be able to get the minimum offer if there are no offers", async () => {
      await ticketSale.connect(addr1).buyTicket({ value: 100 });
      await ticketSale.connect(addr2).buyTicket({ value: 100 });
      await ticketSale.connect(addr1).submitOffer(1, 120);
      await(ticketSale.connect(addr2).submitOffer(2, 80));
      expect (ticketSale.connect(addr3).getMinOffer()).to.be.revertedWith("No offer with minimum price");
  });
  it("should be able to get the minimum price and minimum offer id if there are multiple offers submitted", async () => {
    await ticketSale.connect(addr1).buyTicket({ value: 100 });
    await ticketSale.connect(addr2).buyTicket({ value: 100 });
    await ticketSale.connect(addr1).submitOffer(1, 120);
    await(ticketSale.connect(addr2).submitOffer(2, 80));
    minprice = await ticketSale.minPrice();
    minid = await ticketSale.minTicketId();
    expect(minprice).to.equal(80);
    expect(minid).to.equal(2);
});
